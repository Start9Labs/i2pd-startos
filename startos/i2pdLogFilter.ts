...
/**
 * Filter for the router's log stream at `loglevel = warn`.
 *
 * `warn` is the default because anything quieter suppresses the evidence for
 * real failures — a reseed that cannot complete, a bind that fails, a SAM
 * bridge that stopped serving. But at warn a healthy router also narrates its
 * routine network weather at ~25 lines a minute, which in field exports is
 * >98% of its output and none of it actionable; unfiltered it pushes a
 * 10,000-line log export down to about six hours of history.
 *
 * The list below drops exactly those measured message families and nothing
 * else. Every pattern is anchored to one complete, known-benign message, so a
 * new variant, a different failure, or any line this list has never seen still
...
const CHURN_FAMILIES: RegExp[] = [
  ...
  // SAM per-stream teardown: a client service abandoning its own dial
  // attempts. Bridge-level failures stay visible — "SAM: Bind", "SAM: Accept
  // error" and "SAM: I2P acceptor has been reset" are the only router-side
  // evidence of a SAM bridge that stopped serving, so they are deliberately
  // NOT in this list.
...
// output; pass it through unfiltered rather than buffering it forever.
const CARRY_LIMIT = 65536

/**
 * Returns a chunk handler for one of the daemon's output streams. Chunks are
 * reassembled into lines (a chunk boundary can fall mid-line or mid-codepoint,
 * so each stream needs its own instance), churn lines are dropped, and
 * everything else is written out unchanged.
 */
export const i2pdLogFilter = (out: {
  ...
  // The consumer end of the log pipe is outside our control; a write failure
  // there must not take down the supervisor that owns the router.
